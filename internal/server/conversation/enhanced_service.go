...
import (
	...
	"github.com/koopa0/assistant-go/internal/ai/tokens"
...
func NewEnhancedConversationService(queries *sqlc.Queries, logger *slog.Logger, metrics *observability.Metrics) *EnhancedConversationService {
	return &EnhancedConversationService{
		...
		logger:       logger,
		metrics:      metrics,
		tokenCounter: tokens.NewTokenCounter("claude"), // 預設使用 Claude 模型的 token 計算
	}
}
...
func (s *EnhancedConversationService) GetConversations(ctx context.Context, userID string, archived *bool, limit, offset int32) ([]ConversationSummary, int32, error) {
	...
	// 取得對話列表
	conversations, err := s.queries.GetConversationsByUser(ctx, pghelpers.UUIDToPgtypeUUID(userUUID))
	if err != nil {
	...
	for _, conv := range conversations {
		...
		if err != nil {
			s.logger.Warn("Failed to get message info",
				slog.String("conv_id", pghelpers.PgtypeUUIDToUUID(conv.ID).String()),
				slog.Any("error", err))
		}

		summary := ConversationSummary{
			ID:           pghelpers.PgtypeUUIDToUUID(conv.ID).String(),
			UserID:       pghelpers.PgtypeUUIDToUUID(conv.UserID).String(),
			Title:        conv.Title,
			Summary:      pghelpers.PgtypeTextToStringPtr(conv.Summary),
...
func (s *EnhancedConversationService) CreateConversation(ctx context.Context, userID string, req CreateConversationRequest) (*ConversationDetail, error) {
	...
	conv, err := s.queries.CreateConversation(ctx, sqlc.CreateConversationParams{
		UserID:   pghelpers.UUIDToPgtypeUUID(userUUID),
		Title:    req.Title,
		Summary:  pghelpers.StringPtrToPgtypeText(&req.Summary),
...
func (s *EnhancedConversationService) GetConversation(ctx context.Context, conversationID string) (*ConversationDetail, error) {
	...
	// 取得對話
	conv, err := s.queries.GetConversation(ctx, pghelpers.UUIDToPgtypeUUID(convUUID))
	if err != nil {
	...
	messages, err := s.queries.GetMessagesByConversation(ctx, sqlc.GetMessagesByConversationParams{
		ConversationID: pghelpers.UUIDToPgtypeUUID(convUUID),
		Limit:          1000, // 預設限制
		Offset:         0,
	})
	if err != nil {
	...
	for _, msg := range messages {
		...
		if err != nil {
			s.logger.Warn("Failed to get tool executions",
				slog.String("msg_id", pghelpers.PgtypeUUIDToUUID(msg.ID).String()),
				slog.Any("error", err))
		}

		messageDetail := MessageDetail{
			ID:         pghelpers.PgtypeUUIDToUUID(msg.ID).String(),
			Role:       msg.Role,
			Content:    msg.Content,
			TokenCount: pghelpers.PgtypeInt4ToInt32Ptr(msg.TokenCount),
			Metadata:   metadata,
...
func (s *EnhancedConversationService) SendMessage(ctx context.Context, conversationID string, req SendMessageRequest) (*SendMessageResponse, error) {
	...
	msg, err := s.queries.CreateMessage(ctx, sqlc.CreateMessageParams{
		...
		Content:        req.Content,
		Metadata:       metadataJSON,
		TokenCount:     pghelpers.Int32ToPgtypeInt4(tokenCount),
	})
	if err != nil {
	...
	// 建立對話摘要
	messageCount, lastMessage, _ := s.getConversationMessageInfo(ctx, pghelpers.UUIDToPgtypeUUID(convUUID))
...
func (s *EnhancedConversationService) getConversationMessageInfo(ctx context.Context, conversationID pgtype.UUID) (int32, *MessageSummary, error) {
	...
	lastMessage := &MessageSummary{
		ID:         pghelpers.PgtypeUUIDToUUID(lastMsg.ID).String(),
		Role:       lastMsg.Role,
		Preview:    preview,
		TokenCount: pghelpers.PgtypeInt4ToInt32Ptr(lastMsg.TokenCount),
...
func (s *EnhancedConversationService) getToolExecutions(ctx context.Context, messageID pgtype.UUID) ([]ToolExecutionSummary, error) {
	...
	for _, exec := range executions {
		summary := ToolExecutionSummary{
			ID:              pghelpers.PgtypeUUIDToUUID(exec.ID).String(),
			ToolName:        exec.ToolName,
			Status:          exec.Status,
			ExecutionTimeMs: pghelpers.PgtypeInt4ToInt32Ptr(exec.ExecutionTimeMs),
			StartedAt:       pghelpers.PgtypeTimestamptzToTime(exec.StartedAt),
			CompletedAt:     pghelpers.PgtypeTimestamptzToTimePtr(exec.CompletedAt),
		}
		summaries = append(summaries, summary)
	}
...
func (s *EnhancedConversationService) GetTokenCounter() tokens.Counter {
