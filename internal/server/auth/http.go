...
func (h *HTTPHandler) HandleLogin(w http.ResponseWriter, r *http.Request) {
	...
	if req.Email == "" || req.Password == "" {
		h.writeError(w, "INVALID_REQUEST", "電子郵件和密碼為必填", http.StatusBadRequest)
		return
	}
	...
	if err != nil {
		h.writeError(w, "UNAUTHORIZED", "電子郵件或密碼錯誤", http.StatusUnauthorized)
		return
	}
...
func (h *HTTPHandler) HandleRefresh(w http.ResponseWriter, r *http.Request) {
	...
	if authHeader == "" {
		h.writeError(w, "UNAUTHORIZED", "缺少授權標頭", http.StatusUnauthorized)
		return
	}

	tokenString := strings.TrimPrefix(authHeader, "Bearer ")
	if tokenString == authHeader {
		h.writeError(w, "UNAUTHORIZED", "無效的授權格式", http.StatusUnauthorized)
		return
	}
	...
	if err != nil {
		h.writeError(w, "UNAUTHORIZED", "無效的更新權杖", http.StatusUnauthorized)
		return
	}
...
func (h *HTTPHandler) HandleLogout(w http.ResponseWriter, r *http.Request) {
	...
	if !ok {
		h.writeError(w, "UNAUTHORIZED", "未授權", http.StatusUnauthorized)
		return
	}
...
func (h *HTTPHandler) HandleRegister(w http.ResponseWriter, r *http.Request) {
	...
	if req.Email == "" || req.Password == "" || req.Name == "" {
		h.writeError(w, "INVALID_REQUEST", "所有欄位皆為必填", http.StatusBadRequest)
		return
	}

	// Check password strength
	if len(req.Password) < 8 {
		h.writeError(w, "INVALID_REQUEST", "密碼長度至少需要 8 個字元", http.StatusBadRequest)
		return
	}
	...
	if err != nil {
		if err.Error() == "email already registered" {
			h.writeError(w, "INVALID_REQUEST", "此電子郵件已被註冊", http.StatusBadRequest)
			return
		}
		h.writeError(w, "SERVER_ERROR", "註冊失敗", http.StatusInternalServerError)
		return
	}
...
func (h *HTTPHandler) HandleForgotPassword(w http.ResponseWriter, r *http.Request) {
	...
	if req.Email == "" {
		h.writeError(w, "INVALID_REQUEST", "電子郵件為必填", http.StatusBadRequest)
		return
	}

	// Always return success to prevent email enumeration
	// In a real implementation, send reset email if user exists
	if h.service.mockUserExists(req.Email) {
...
func (h *HTTPHandler) HandleResetPassword(w http.ResponseWriter, r *http.Request) {
	...
	if req.Token == "" || req.NewPassword == "" {
		h.writeError(w, "INVALID_REQUEST", "權杖和新密碼為必填", http.StatusBadRequest)
		return
	}

	// Validate reset token
	if !h.service.validateResetToken(req.Token) {
		h.writeError(w, "INVALID_REQUEST", "無效或過期的重設權杖", http.StatusBadRequest)
		return
	}
	...
	if err != nil {
		h.writeError(w, "SERVER_ERROR", "密碼重設失敗", http.StatusInternalServerError)
		return
	}
...
func (h *HTTPHandler) HandleVerifyEmail(w http.ResponseWriter, r *http.Request) {
	...
	if token == "" {
		h.writeError(w, "INVALID_REQUEST", "缺少驗證權杖", http.StatusBadRequest)
		return
	}

	// Verify token
	if !h.service.validateEmailToken(token) {
		h.writeError(w, "INVALID_REQUEST", "無效或過期的驗證權杖", http.StatusBadRequest)
		return
	}
