...
func TestPromptService_EnhanceQuery(t *testing.T) {
	...
	service := prompts.NewPromptService(logger)
	...
	testCases := []struct {
	...
	}{
		{
			...
			context:       &prompts.PromptContext{ProjectType: "microservice"},
		...
		},
		{
			...
			context:       &prompts.PromptContext{ProjectType: "library"},
		...
		},
		{
			...
			context:       &prompts.PromptContext{ProjectType: "web", FileName: "handler.go"},
		...
		},
		{
			...
			context:       &prompts.PromptContext{ProjectType: "API"},
		...
		},
		{
			...
			context:       &prompts.PromptContext{ProjectType: "CLI"},
		...
		},
		{
			...
			context:       &prompts.PromptContext{ProjectType: "service", ErrorMessage: "runtime error: invalid memory address"},
		...
		},
		{
			...
			context:       &prompts.PromptContext{ProjectType: "monorepo", ModulePath: "github.com/example/project"},
			expectedType:  "code_analysis",
			minConfidence: 0.1,
...
func TestPromptService_TaskTypeDetection(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.description, func(t *testing.T) {
			enhanced, err := service.EnhanceQuery(ctx, tc.query, &prompts.PromptContext{})
			require.NoError(t, err)
