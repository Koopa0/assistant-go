...
func TestRegistryGetTool(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			config:   &ToolConfig{Timeout: 30 * time.Second},
		},
		{
...
func TestRegistryExecute(t *testing.T) {
	...
	successFactory := func(config *ToolConfig, logger *slog.Logger) (Tool, error) {
		return &testTool{
			name:          "success_tool",
			executeResult: &ToolResult{Success: true, Data: &ToolResultData{Output: map[string]interface{}{"result": "test result"}}},
		}, nil
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantError {
				...
				if tt.wantData != "" && result.Data != nil && result.Data.Output != nil {
					if resultValue, exists := result.Data.Output["result"]; exists {
						if resultValue != tt.wantData {
							t.Errorf("Execute() Data[\"result\"] = %v, want %v", resultValue, tt.wantData)
						}
					}
				}
...
func (t *testTool) Execute(ctx context.Context, input *ToolInput) (*ToolResult, error) {
	...
	return &ToolResult{Success: true, Data: &ToolResultData{Output: map[string]interface{}{"result": "test result"}}}, nil
}
...
func (s *slowTool) Parameters() *ToolParametersSchema {
	return &ToolParametersSchema{Type: "object", Properties: map[string]ParameterProperty{}, Required: []string{}}
}
