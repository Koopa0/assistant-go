...
func NewLangChainToolAdapter(internalTool Tool, logger *slog.Logger) *LangChainToolAdapter {
...
func (a *LangChainToolAdapter) formatResult(result *ToolResult) (string, error) {
	if !result.Success {
		return fmt.Sprintf("Error: %s", result.Error), nil
	}

	// If data contains output, return it as JSON
	if result.Data != nil && result.Data.Output != nil && len(result.Data.Output) > 0 {
		outputJSON, err := json.Marshal(result.Data.Output)
		if err != nil {
			return fmt.Sprintf("Output: %v", result.Data.Output), nil
		}
...
func NewToolRegistry(internalRegistry *Registry, logger *slog.Logger) *ToolRegistry {
	return &ToolRegistry{
