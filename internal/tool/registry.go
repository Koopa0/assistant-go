// Package tool provides a flexible tool registry and execution framework.
// It supports dynamic tool registration, execution with context awareness,
// and includes built-in tools for Go development, Docker, PostgreSQL, and more.
package tool
...
func (r *Registry) Stats(ctx context.Context) (*RegistryStats, error) {
	...
	for name := range r.factories {
		...
		toolStats[name] = &ToolStats{
			TotalExecutions:   0,           // TODO: Implement execution tracking
			SuccessfulRuns:    0,           // TODO: Implement success tracking
			FailedRuns:        0,           // TODO: Implement error tracking
			AverageRunTime:    0,           // TODO: Implement timing tracking
			LastExecutionTime: time.Time{}, // TODO: Implement execution tracking
			ErrorRate:         0,           // 0% error rate until tracking is implemented
			IsHealthy:         true,        // Default to healthy
		}
	}

	stats := &RegistryStats{
		TotalTools:      len(r.factories),
		EnabledTools:    activeCount,
		DisabledTools:   len(r.factories) - activeCount,
		ToolsByCategory: r.countByCategory(),
		ExecutionStats:  toolStats,
		LastHealthCheck: time.Now(),
		HealthyTools:    activeCount, // TODO: Implement actual health checking
		UnhealthyTools:  0,           // TODO: Implement actual health checking
	}

	return stats, nil
}

// countByCategory counts tools by category
...
func (r *Registry) StatsLegacy(ctx context.Context) (map[string]interface{}, error) {
	...
	legacyStats := map[string]interface{}{
		"registered_factories": stats.TotalTools,
		"active_instances":     stats.EnabledTools,
		"disabled_tools":       stats.DisabledTools,
		"tools_by_category":    stats.ToolsByCategory,
		"last_health_check":    stats.LastHealthCheck.Format(time.RFC3339),
		"healthy_tools":        stats.HealthyTools,
		"unhealthy_tools":      stats.UnhealthyTools,
		"tools":                make(map[string]interface{}),
	}

	// Convert tool stats
	toolStats := make(map[string]interface{})
	for name, stat := range stats.ExecutionStats {
		toolStats[name] = map[string]interface{}{
			"total_executions":    stat.TotalExecutions,
			"successful_runs":     stat.SuccessfulRuns,
			"failed_runs":         stat.FailedRuns,
			"average_run_time":    stat.AverageRunTime.String(),
			"last_execution_time": stat.LastExecutionTime,
