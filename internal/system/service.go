...
// Service handles system monitoring and management logic
type Service struct {
...
func NewService(assistant *assistant.Assistant, queries sqlc.Querier, logger *slog.Logger, metrics *observability.Metrics) *Service {
	return &Service{
		assistant:       assistant,
		queries:         queries,
...
func (s *Service) GetStatus(ctx context.Context) (*SystemStatus, error) {
	// Track request
...
func (s *Service) GetActivities(ctx context.Context, activityType, startDate, endDate string, page, limit int) ([]SystemActivity, int, error) {
...
func (s *Service) GetPerformance(timeRange string) map[string]interface{} {
	if timeRange == "" {
		timeRange = "1h"
	}
...
func (s *Service) TrackError() {
	atomic.AddInt64(&s.totalErrors, 1)
}

// SetActiveUsers updates the active user count
func (s *Service) SetActiveUsers(count int64) {
	atomic.StoreInt64(&s.activeUsers, count)
}

// Helper methods

func (s *Service) checkServices(ctx context.Context) map[string]string {
...
func (s *Service) calculateMemoryUsage() float64 {
	var memStats runtime.MemStats
	runtime.ReadMemStats(&memStats)
...
func (s *Service) calculateAverageLatency() float64 {
	// Mock average latency
	return 25.5
}

func (s *Service) updateRequestsPerMinute() {
	now := time.Now()
...
func (s *Service) getDatabaseMetrics(ctx context.Context) map[string]interface{} {
...
func (s *Service) generateSampleActivities(activityType, startDate, endDate string, page, limit int) []SystemActivity {
