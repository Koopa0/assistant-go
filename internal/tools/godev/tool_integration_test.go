...
func TestGoDevTool_Integration(t *testing.T) {
	...
	tool := godev.NewGoDevTool(logger)
	...
	t.Run("AnalyzeWorkspace", func(t *testing.T) {
		...
		if result.Data != nil && result.Data.Output != "" {
			assert.Contains(t, result.Data.Output, "Project Type:")
			assert.Contains(t, result.Data.Output, "Main Package:")
		}
	})
