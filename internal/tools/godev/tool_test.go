...
func TestGoDevTool_Basic(t *testing.T) {
	...
	// Create the tool
	tool := NewGoDevTool(logger)

	// Test basic properties
	if tool.Name() != "godev" {
		t.Errorf("Expected name 'godev', got '%s'", tool.Name())
	}
...
func TestGoDevTool_JSONInput(t *testing.T) {
	...
	result, err := tool.ExecuteWithJSON(context.Background(), jsonInput)
	if err != nil {
...
func TestGoDevTool_InvalidJSON(t *testing.T) {
	...
	result, err := tool.ExecuteWithJSON(context.Background(), invalidJSON)
	if err != nil {
		t.Fatalf("ExecuteWithJSON failed: %v", err)
	}
