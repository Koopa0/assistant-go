...
import (
	...
	"github.com/koopa0/assistant-go/internal/tools/postgres"
)
...
func ExamplePostgresTool_analyzeQuery() {
	...
	if result.Success {
		...
		if err := json.Unmarshal([]byte(result.Data.Output), &analysis); err == nil {
...
func ExamplePostgresTool_validateMigration() {
	...
	if result.Success {
		...
		if err := json.Unmarshal([]byte(result.Data.Output), &validation); err == nil {
