...
func (a *BaseAgent) executeLLM(ctx context.Context, req Request) (string, error) {
	...
	// Add any context as a system message if needed
	if req.Context != nil && len(req.Context) > 0 {
		...
		contextStr := fmt.Sprintf("Context: %v", req.Context)
		opts = append(opts, llms.WithSystemPrompt(contextStr))
	}

	// Call LLM
	result, err := a.llm.Call(ctx, req.Query, opts...)
	if err != nil {
